...
class ResizeImgLine(object):
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            if self.obj_rep == 'XYXYSin2':
              ...
              bboxes = results[key][:,:3] * results['scale_factor']
              bboxes[:, 0] = np.clip(bboxes[:, 0], 0, img_shape[1] - 1)
              bboxes[:, 1] = np.clip(bboxes[:, 1], 0, img_shape[0] - 1)
              results[key][:,:3] = bboxes
              results[key][:,[6,7]] *= results['scale_factor']
            else:
    ...
    def __call__(self, results):
        ...
        #_show_objs_ls_points_ls(results['img'][:,:,0], [results['gt_bboxes']], self.obj_rep)
        return results
...
class RandomLineFlip(object):
    ...
    def bbox_flip_XYXYSin2(self, bboxes, img_shape, direction, obj_rep):
        if obj_rep == 'XYXYSin2':
...
@PIPELINES.register_module
class PadToSameHW_ForRotation(object):
    """Pad the image & mask.
    ...
    def __init__(self, obj_rep, pad_border_make_bboxes_pos=True,  pad_val=0):
        self.obj_rep = obj_rep
        self.pad_val = pad_val
    ...
    def _pad_img(self, results):
        ...
        box_boader = int(max(0, -gt_bboxes[:,:4].min())+2)
        ...
        min_border = 3
        border_pad = np.clip(min_border - min_corners, a_min=box_boader, a_max=None)
        ...
        wpad0 = max((w1-w0)//2, border_pad[0])
        ...
        size = (h1, w1, results['img'].shape[2])
        results['img'] = padded_img
        results['pad_shape'] = padded_img.shape
        ...
        if results['rotate']:
            ...
            for key in results.get('bbox_fields', []):
                if results[key].shape[0] == 0:
                  continue
                results[key], results['img'] = rotate_bboxes_img(results[key],
