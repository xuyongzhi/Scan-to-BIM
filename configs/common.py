...
class DIM_PARSE:
  ...
  def __init__(self, num_classes, is_out_corner=False):
    '''
    ...
    SCORE_REFINE_DIM = self.NUM_CLASS
    SCORE_FINAL_DIM = self.NUM_CLASS
    OUT_EXTAR_DIM = POINTS_DIM + OBJ_DIM + SCORE_REFINE_DIM + SCORE_FINAL_DIM # see OUT_ORDER
    NMS_IN_DIM = self.OBJ_DIM + self.OUT_EXTAR_DIM
    CORNER_DIM = 4
    AVE_LINE_SCORE = self.NUM_CLASS
    COMPOSITE_SCORE = self.NUM_CLASS

    IS_OUT_CORNER = self.is_out_corner
